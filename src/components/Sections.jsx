    ...
    <section className="relative bg-slate-950 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-20 sm:py-24">
        <div className="grid gap-10 md:grid-cols-2 md:gap-16">
          <div>
            <h2 className="mb-3 sm:mb-4 text-2xl sm:text-3xl md:text-4xl font-semibold">Philosophy</h2>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 sm:gap-4">
              {[
                ...
                >
                  <div className="text-2xl sm:text-3xl font-semibold" aria-label={`${s.value} ${s.label}`}>
                    {s.value}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {['Design Systems', 'Motion Design', 'Frontend', 'Accessibility', 'Prototyping', '3D/Visuals'].map((skill) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/0 p-4 sm:p-5"
              >
                <div className="mb-2 text-xs sm:text-sm text-white/60">Capability</div>
                <div className="text-base sm:text-lg font-medium">{skill}</div>
    ...
    <section className="relative bg-slate-950 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-20 sm:py-24">
        <div className="mb-6 sm:mb-8 flex items-end justify-between gap-4">
          ...
          <a className="text-xs sm:text-sm text-cyan-300 hover:text-cyan-200" href="#">View All</a>
        </div>
          ...
          {projects.map((p, idx) => (
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
                <div className="mb-1.5 sm:mb-2 text-base sm:text-lg font-medium">{p.title}</div>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {p.tags.map((t) => (
                    <span key={t} className="rounded-full border border-white/15 bg-white/10 px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs text-white/80">{t}</span>
                  ))}
                </div>
              </div>
            </motion.a>
    ...
    <section id="contact" className="relative bg-slate-950 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-20 sm:py-24">
        <div className="mx-auto max-w-2xl">
          <h2 className="mb-2 text-center text-2xl sm:text-3xl md:text-4xl font-semibold">Let’s build something remarkable</h2>
          <p className="mb-6 sm:mb-8 text-center text-white/70">Briefly share your goals and I’ll get back within 24 hours.</p>

          <form
            onSubmit={(e) => { e.preventDefault(); alert('Message sent!') }}
            className="grid gap-3 sm:gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 sm:p-6 backdrop-blur"
          >
            <div className="grid gap-1.5 sm:gap-2">
              <label htmlFor="name" className="text-xs sm:text-sm text-white/70">Name</label>
            ...
            <div className="grid gap-1.5 sm:gap-2">
              <label htmlFor="email" className="text-xs sm:text-sm text-white/70">Email</label>
            ...
            <div className="grid gap-1.5 sm:gap-2">
              <label htmlFor="msg" className="text-xs sm:text-sm text-white/70">Message</label>
